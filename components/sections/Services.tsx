    ...
    <section id="services" className="section-shell py-12 md:py-16">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {items.map((item) => (
          <article
            key={item.title}
            className="overflow-hidden rounded-[24px] bg-card soft-shadow border border-border"
          >
            ...
            <div className="p-5">
              ...
              <h3 className="mt-2 font-display text-[20px] font-bold text-text-secondary">
                {item.title}
              </h3>
              <p className="mt-2 text-[14px] leading-7 text-text-muted">{item.description}</p>
              <a
                href="#contact"
                className="mt-5 flex w-full items-center justify-center rounded-[16px] bg-accent px-5 py-3 text-[14px] font-bold text-accent-foreground transition-transform hover:-translate-y-0.5"
              >
                {t('learnMore')}
              </a>
            </div>
